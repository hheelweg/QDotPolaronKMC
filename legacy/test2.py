...
w_c = 0.05                                   # cutoff frequency (units?)
J_c = 1                                    # J_c (units?)
inhomog_sd = 0.2                          # inhomogenous broadening (units?)
nrg_center = 2.0                            # mean site energy (units ?)
rel_spatial_disorder = 0.2                  # relative spatial disorder
...
inhomog_sds = np.logspace(-4, -1, 10)
polaron_sds = np.zeros(10)
iprs = np.zeros(10)

for i in np.arange(len(inhomog_sds)):
    kmc_setup = mc.KMCRunner(ndim, N, spacing, nrg_center, inhomog_sds[i], dipolegen, seed, rel_spatial_disorder,
                                J_c, spectrum, temp, ntrajs, r_hop, r_ove, r_box)
    
    
    # get polaron states
    polaron_states = kmc_setup.polaron_locs
    polaron_sds[i] = (np.std(kmc_setup.eignrgs))
    iprs[i] = kmc_setup.get_ipr()
    ...
    # plt.legend()
    # plt.show()
plt.loglog(inhomog_sds, inhomog_sds)
plt.loglog(inhomog_sds, polaron_sds)
plt.xlabel("inhomog sds (eV)")
plt.ylabel("polaron sds (eV)")

plt.show()
plt.semilogx(inhomog_sds, iprs)
plt.xlabel("inhomog sds")
plt.ylabel("iprs")
plt.show()
# %%
temps = np.linspace(1, 1000, 20)
inhomog_sd = 0.01
polaron_sds = np.zeros(20)
iprs = np.zeros(20)
for i in np.arange(len(temps)):
    kmc_setup = mc.KMCRunner(ndim, N, spacing, nrg_center, inhomog_sd, dipolegen, seed, rel_spatial_disorder,
                                J_c, spectrum, temps[i], ntrajs, r_hop, r_ove, r_box)
    ...
    # plt.legend()
    # plt.show()
plt.semilogy(temps, polaron_sds)
plt.xlabel("temp")
plt.ylabel("polaron sds")
plt.show()
plt.plot(temps, iprs)
plt.xlabel("temp")
plt.ylabel("iprs")
