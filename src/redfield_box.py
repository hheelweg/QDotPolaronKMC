...
class NewRedfield(Unitary):
    ...
    def make_redfield_box_global_exact(self, *, pol_idxs_global, site_idxs_global, center_global, debug=False):
        import time
        t_all = time.time()
        tv = getattr(self, "time_verbose", False)

        # Force clean caches (debug)
        if hasattr(self, "_lam_idx_cache"): del self._lam_idx_cache
        if hasattr(self, "_A_lambda_cache"): del self._A_lambda_cache

        pol_g  = np.asarray(pol_idxs_global,  dtype=np.intp)
        site_g = np.asarray(site_idxs_global, dtype=np.intp)
        npols, nsites = pol_g.size, site_g.size
        if tv: print('npols, nsites', npols, nsites)

        # map center -> local index
        gl2loc = {int(g): i for i,g in enumerate(pol_g)}
        center_i = int(gl2loc[int(center_global)])

        # ==== build λ-index cache exactly as baseline ====
        if not hasattr(self, "_lam_idx_cache"):
            self._lam_idx_cache = {}
        lamdalist = (-2.0, -1.0, 0.0, 1.0, 2.0)
        if nsites not in self._lam_idx_cache:
        ...
        # ==== CSR A_λ cache exactly as baseline ====
        if not hasattr(self, "_A_lambda_cache"):
            self._A_lambda_cache = {}
        if nsites not in self._A_lambda_cache:
            from scipy import sparse
        ...
        # ==== Bath integrals: scalar calls, baseline ω indexing ====
        t0 = time.time()
        bath_integrals = []
        for lam in lamdalist:
            vec = np.zeros(npols, dtype=np.complex128)
            if lam != 0.0:
                for i_local in range(npols):
                    # BASELINE QUIRK: local row index into global omega_diff
                    omega_ij = self.ham.omega_diff[i_local, int(center_global)]
                    # scalar path:
                    vec[i_local] = self.ham.spec.correlationFT(float(omega_ij), float(lam), float(self.kappa))
            bath_integrals.append(vec)
        if tv: print('time(bath integrals)', time.time() - t0, flush=True)

        # ==== Transform full, then slice (baseline) ====
        t1 = time.time()
        Gs = np.empty((nsites, nsites, npols, npols), dtype=np.complex128)
        for aa, a_idx in enumerate(site_g):
            for bb, b_idx in enumerate(site_g):
                G_full = self.ham.site2eig(self.ham.sysbath[int(a_idx)][int(b_idx)])
                Gs[aa, bb] = G_full[np.ix_(pol_g, pol_g)]
        if tv: print('time(site→eig)', time.time() - t1, flush=True)

        # ==== R, C as baseline; NO PRUNE (for equality check) ====
        AB = nsites * nsites
        R = np.ascontiguousarray(Gs[:, :, center_i, :].reshape(AB, npols))
        C = np.ascontiguousarray(Gs[:, :, :, center_i].reshape(AB, npols))

        # ==== gamma via CSR×dense + einsum (same order) ====
        from scipy import sparse
        t2 = time.time()
        gamma_plus = np.zeros(npols, dtype=np.complex128)
        for lam_idx, lam in enumerate(lamdalist):
            ...
            contrib = np.einsum('an,an->n', R, Y, optimize=True)
            gamma_plus += bath_integrals[lam_idx] * contrib
        if tv: print('time(gamma accumulation)', time.time() - t2, flush=True)

        red_R_tensor = 2.0 * np.real(gamma_plus)
        rates = np.delete(red_R_tensor, center_i) / const.hbar
        final_site_idxs = np.delete(pol_g, center_i).astype(int)

        if tv: print('time(total)', time.time() - t_all, flush=True)
        if debug:
            return dict(
                pol_g=pol_g.copy(), site_g=site_g.copy(),
                center_i=center_i, center_global=int(center_global),
                bath=[b.copy() for b in bath_integrals],
                R=R.copy(), C=C.copy(),
